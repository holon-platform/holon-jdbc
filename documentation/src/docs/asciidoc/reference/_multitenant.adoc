...
. A link:{apidir}/com/holonplatform/jdbc/TenantDataSourceProvider.html[TenantDataSourceProvider^], which acts as a concrete `DataSource` instances provider, configured at DataSource build time or available in platform `Context`.
