...
So, if you configured two DataSources using for example the following properties in the `application.properties` file:

[source, text]
----
holon.datasource.one.url=jdbc:h2:mem:testdb1
holon.datasource.one.username=sa
holon.datasource.one.password=

holon.datasource.two.url=jdbc:h2:mem:testdb2
holon.datasource.two.username=sa
holon.datasource.two.password=
----

You can provide different initialization scripts for each `DataSource`, i.e. for each _data context id_, named in this example `one` and `two`. For the `one` DataSource you will provide the initialization files `one-schema.sql` and `one-data.sql`, while for the `two` DataSource you will provide the initialization files `two-schema.sql` and `two-data.sql`
